...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_FILES["archivo_banner"])) {
        $nombreArchivo = $_FILES["archivo_banner"]["name"];
        $tipoArchivo = $_FILES["archivo_banner"]["type"];
...
function guardarUsuarioEnJSON($usuario) {
    ...
    if ($claveUsuario !== false) {
        $usuarios[$claveUsuario]['banner'] = $usuario['banner'];
    }
